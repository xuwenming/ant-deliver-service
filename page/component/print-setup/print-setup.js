...
Page({
  ...
  lanya7: function () {
      ...
      success: function (res) {
        for (var i = 0; i < res.characteristics.length; i++) {
          if (res.characteristics[i].properties.notify) {
            console.log("11111111", that.data.services[1].uuid);
    ...
    //       if (res.characteristics[i].properties.notify) {
    //         that.setData({
    //           notifyServicweId: that.data.services[2].uuid,
    //           notifyCharacteristicsId: res.characteristics[i].uuid,
    //         })
    //       }
    //       if (res.characteristics[i].properties.write) {
    //         that.setData({
    //           writeServicweId: that.data.services[2].uuid,
    //           writeCharacteristicsId: res.characteristics[i].uuid,
    //         })

    //       } else if (res.characteristics[i].properties.read) {
    //         that.setData({
    //           readServicweId: that.data.services[2].uuid,
    //           readCharacteristicsId: res.characteristics[i].uuid,
    //         })

    //       }
    //     }
    //     console.log('device getBLEDeviceCharacteristics2:', res.characteristics);

    //     that.setData({
    //       msg2: JSON.stringify(res.characteristics),
    //     })
    //   },
    //   fail: function () {
    //     console.log("fail2");
  ...
  lanya8: function () {
    ...
    request.httpGet({
      url: config.getEncrypt,  
      success: function (data) {
        for(var i in data.obj) {
          var typedArray = new Uint8Array(data.obj[i].match(/[\da-f]{2}/gi).map(function (h) {
            return parseInt(h, 16)
          }))

          that.writeBLECharacteristicValue(typedArray);
        }
        
        // console.log(typedArray);

        // var len = parseInt(typedArray.length/100) + 1;
        // console.log(len);
        // for (var i = 0; i < len; i++) {
        //   var arr = typedArray.slice(i*100, 100*(i+1) - 1);
        //   console.log(arr);
        //   that.writeBLECharacteristicValue(arr);
        // }
  ...
  writeBLECharacteristicValue: function (arr){
    var that = this;
